...
- **`CargoContrato` — modelo nuevo** (`@@map("cargos_contrato")`): cargo one-off sobre un contrato — `tipo TipoCargo`, `concepto`, `monto Decimal(14,2)`, `moneda`, `creadoPorId`. **Ahora se LEE** (08/07, migración `cargo_saldado`): gana `saldadoAt DateTime?` + `saldadoPorId` — un cargo saldado por la inmo (`POST /cargos/:id/saldar` o `saldar-deuda`) deja de ser deuda. `GET /mis-cargos` lo expone al inquilino (excluye `contraDeposito` y saldados) y `GET /contratos/:id/cargos` al panel. El saldo por liquidaciones sigue en `lib/saldos.ts`; los cargos son deuda aparte (no se pagan por checkout, se coordinan con la inmo).
