...
| `APP_ADMIN_URL` | No | `https://admin.myalquiler.com` | Railway service `api` | Base de los links al panel en los emails a la inmobiliaria (bienvenida, invitación al equipo, reclamo nuevo). Se le saca la barra final. |
| `APP_INQUILINO_URL` | No | `https://app.myalquiler.com` | Railway service `api` | Base de los links a la PWA del inquilino en los emails (anuncios, aviso de aumento, reclamos). Se le saca la barra final. |
| `APP_PROPIETARIO_URL` | No | `https://admin.myalquiler.com/propietario` | Railway service `api` | Base del link al **portal del propietario** en los emails que le llegan (hoy: los anuncios con audiencia `TODOS_PROPIETARIOS`). ⚠️ El portal **no tiene servicio propio**: se sirve como export estático desde el host del **panel**, bajo `/propietario` (ver [`work-agent/02-DEPLOY.md`](../work-agent/02-DEPLOY.md)). Si se setea, tiene que apuntar ahí — un dominio propio del portal no existe. |
...
| `UPLOADS_CUOTA_MB` | No | `2048` (2 GB) | Railway service `api` | Cuota de disco **por inmobiliaria** en `POST /uploads` (`apps/api/src/lib/cuota-uploads.ts`). El Volume es uno solo y compartido: sin esto, un token vivo —el de un inquilino dura 15 días, incluso con el contrato terminado— podía llenarlo y dejar sin subir a todos los tenants. Al llegar al tope el endpoint devuelve **507** con `codigo: CUOTA_TENANT_LLENA`. Un valor vacío o inválido **no** la apaga: cae al default. Para desactivarla hay que escribir `0` a propósito. |
