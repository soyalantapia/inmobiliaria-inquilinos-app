...
- ✅ **EXPEDIENTE DE VIDA** (08/07, "el flujo completo / trackeo") — 3 endpoints nuevos `/propiedades/:id/{salud-pago,seguros,timeline}` + pestaña **Expediente** en la ficha:
  - **Salud de pago**: por contrato (actual + histórico) deuda impaga que quedó (por moneda), cuotas vencidas, comportamiento de pago (a tiempo/tarde, días de atraso, % puntualidad) + depósito. Es "la propiedad tuvo morosos/pagos". Reusa `conSaldo`/`montoPagadoPorLiquidacion`, deriva mora on-read.
  - **Seguros/garantías**: la caución que ya vive en `Garante` (nº/cobertura/vigencia) agregada + estado VIGENTE/POR_VENCER/VENCIDA + alertas. **Decisión del dueño: mostrar lo que existe, sin modelo nuevo.**
  - **Línea de tiempo**: hitos de todos los contratos (inicio, fin/rescisión, ajustes, renovaciones, reclamos, aviso de egreso).
  - Fix de honestidad en la PWA del inquilino: "Pagado a tiempo" hardcodeado → puntualidad real. Verificado E2E. Backend `eea4966`, front `fb2af4a`. GOTCHA: `app.ts` colisionó (la otra sesión metió `miInmobiliariaRoutes` sin commitear) → "app.ts dance" (reset a HEAD + mis regs + commit + restaurar sus 2 líneas).
- ✅ **EXPEDIENTE COMPLETO (6/6)** — se sumaron **gastos/mantenimiento** (`GET /propiedades/:id/gastos`: MovimientoCaja tipo GASTO por propiedad, total + desglose por categoría; card en la pestaña Expediente) y **documentos** (`GET /propiedades/:id/documentos`: DocumentoContrato de todos los contratos, con descarga; reemplaza el placeholder del tab Documentos). Backend `ca25da9`, front `7992867`. Verificado E2E (gastos prp_001 PLOMERIA $45k; documentos prp_004 DNI). ⚠️ **Disco del user en ~700 MB (100%)** → un `railway up` local/build puede fallar; liberar espacio urgente.
