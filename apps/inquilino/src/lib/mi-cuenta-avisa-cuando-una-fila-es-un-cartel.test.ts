 ...
 *
 * El tour de onboarding tenía este defecto y se cerró (#154, `el-tour-no-promete-pantallas-vacias`).
 ...
 * llega a prometer técnicos «verificados». Las dos son carteles de «Disponible pronto» en
 * producción.
 *
 * Y pesa más acá que en el tour: el tour se ve una vez y se saltea; Mi Cuenta es el menú
 * permanente, donde el inquilino vuelve a buscar lo que no encontró.
 *
 * LA DECISIÓN ES DISTINTA A LA DEL TOUR, a propósito. Allá el slide se saca —una pantalla entera
 * dedicada a algo que no existe es puro relleno—. Acá la fila SE QUEDA, con un badge «Pronto»: la
 ...
 *
 * El detector de rutas-cartel es el mismo de #154. Estaba adentro de aquel test y se movió a
 * `lib/pantallas-gateadas.ts` para que los dos controles usen UNA implementación: dos copias de
 * esa regla se desincronizan, que es el defecto que este repo ya se comió varias veces.
 */
import { describe, it, expect } from 'vitest';
import { readFileSync } from 'node:fs';
import { fileURLToPath } from 'node:url';
import { GATEADAS, sinComentarios } from './pantallas-gateadas';
...
function ramaDeProduccion(): string {
  const src = sinComentarios(readFileSync(CUENTA, 'utf8'));
...
describe('Mi Cuenta avisa cuando una fila lleva a un cartel', () => {
  it('CONTROL DEL CONTROL — hay rutas-cartel y Mi Cuenta ofrece alguna', () => {
    ...
    // se pondría verde para siempre. Que avise en vez de callarse.
    expect(GATEADAS.length).toBeGreaterThan(0);
    const real = ramaDeProduccion();
    const ofrecidas = GATEADAS.filter((r) => real.includes(`href="${r}"`));
    expect(ofrecidas.length, 'Mi Cuenta ya no ofrece ninguna ruta-cartel: revisá si este test sigue teniendo sentido').toBeGreaterThan(0);
  });
  ...
  it('🔴 cada fila que lleva a un cartel está marcada con `pronto`', () => {
    ...
    const sinMarcar = GATEADAS.filter((ruta) => {
      ...
      if (i === -1) return false;
      // La ventana cubre lo que queda de esa fila hasta el cierre del componente.
