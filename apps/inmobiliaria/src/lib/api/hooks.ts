...
/**
 * Manda un reporte del piloto AL SERVIDOR.
 *
 * 🔴 Antes no existía, y el FAB del piloto guardaba en `localStorage` con un `setTimeout` de
 * 600 ms comentado como «pequeño delay simulado para que se sienta como una llamada al back».
 * El diálogo prometía, textual, «lo que reportes acá lo lee directo el equipo de producto el
 * mismo día», y el reporte moría en el navegador de quien lo escribió — nadie lo leía nunca, ni
 * siquiera del lado del panel: `listarReportes()` no tenía un solo consumidor.
 *
 * El endpoint existía hace tiempo (`POST /reportes`), autenticado y con el tracking capturado
 * server-side —IP, userAgent, sesión, build—, que es literalmente el TODO que
 * `piloto-storage.ts` dejó escrito.
 *
 * Los headers `x-session-id` y `x-app-build` los manda ESTE hook y no `apiFetch`, que no los
 * conoce. El endpoint ya los lee: sin ellos, dos reportes de la misma sesión no se pueden
 * agrupar y no se sabe contra qué build se reportó — que es la mitad de reproducir un bug.
 */
export function useEnviarReporte(): {
  enviar: (r: {
    ...
    titulo: string;
    detalle?: string;
    severidad?: 'BLOQUEA' | 'MOLESTO' | 'MENOR';
    url?: string;
    urlCompleta?: string;
    viewport?: string;
  }) => Promise<void>;
...
/**
 * Sesión y build del reporte. Los dos son best-effort: si el navegador bloquea el storage o la
 * meta no está, se manda lo que haya. Un reporte sin build es peor que uno con build, pero es
 * muchísimo mejor que ninguno — por eso nada de esto puede tirar.
 */
function cabecerasDeReporte(): Record<string, string> {
  const h: Record<string, string> = {};
  if (typeof window === 'undefined') return h;
  try {
    const K = 'llave:reporte-session';
    ...
    /* modo privado o storage bloqueado: el reporte igual sale */
  }
  const build = document.querySelector('meta[name="build-commit"]')?.getAttribute('content');
  if (build) h['x-app-build'] = build;
  return h;
}
