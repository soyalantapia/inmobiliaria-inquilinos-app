...
import { formatMonto, formatFechaCorta, formatTotalPorMoneda } from '@/lib/format';
import {
  useCuentas,
...
/** El contenido, sin Topbar ni <main>, para poder montarlo también dentro de /caja. */
  ...
  return (
    <Card className={archivada ? 'opacity-70' : ''}>
      <CardContent className="space-y-3 py-4">
        ...
        {/* Todo desglosado por moneda: antes se sumaban pesos y dólares en un solo número
            y se rotulaba en pesos. `formatTotalPorMoneda` con una sola moneda se ve igual
            que antes, así que el caso normal no cambia. */}
        <div className="flex items-end justify-between">
          <div className="space-y-0.5 text-xs">
            {cuenta.direccion !== 'SALIDA' && (
              <p className="flex items-center gap-1 text-emerald-600 dark:text-emerald-400">
                <ArrowDownLeft className="h-3.5 w-3.5" />{' '}
                {formatTotalPorMoneda(cuenta.porMoneda.map((p) => ({ monto: p.entradas, moneda: p.moneda })))}
              </p>
            )}
            {cuenta.direccion !== 'ENTRADA' && (
              <p className="flex items-center gap-1 text-destructive">
                <ArrowUpRight className="h-3.5 w-3.5" />{' '}
                {formatTotalPorMoneda(cuenta.porMoneda.map((p) => ({ monto: p.salidas, moneda: p.moneda })))}
              </p>
            )}
          </div>
          <div className="text-right">
            <p className="text-[11px] uppercase tracking-wide text-muted-foreground">Saldo</p>
            {/* Rojo si CUALQUIER moneda quedó en negativo: con varias monedas no hay un
                único signo, y que una esté en rojo es justamente lo que hay que ver. */}
            <p
              className={`text-lg font-bold tabular-nums ${
                cuenta.porMoneda.some((p) => p.saldo < 0) ? 'text-destructive' : ''
              }`}
            >
              {formatTotalPorMoneda(cuenta.porMoneda.map((p) => ({ monto: p.saldo, moneda: p.moneda })))}
            </p>
          </div>
        </div>
      </CardContent>
    ...
    <Dialog open onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>Movimientos de esta cuenta · saldo {formatTotalPorMoneda(cuenta.porMoneda.map((p) => ({ monto: p.saldo, moneda: p.moneda })))}</DialogDescription>
        </DialogHeader>
