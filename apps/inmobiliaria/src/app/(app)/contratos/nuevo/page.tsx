...
// que alimenta el semáforo del inquilino en el paso 2 del alta.
...
//
// Documentación y Servicios van DESPUÉS del 5 a propósito: metidos antes,
// correrían el paso condicional un lugar y el salto `sig === 5 && !hayPeriodos`
// saltearía el paso equivocado, en silencio.
type PasoApi = 1 | 2 | 3 | 4 | 5 | 6 | 7 | 8;
...
function CargarContratoApiWizard() {
  ...
  /**
   * Si el operador llegó al paso 7, el panel de servicios pudo haber escrito ya
   ...
   */
  const serviciosVisitados = paso >= 7;
  ...
  // Documentación (paso 6). Los papeles elegidos van en UN objeto indexado por
  ...
  // Lo mismo que muestra el paso 6, para el resumen del paso 8: la misma
  ...
  const aplicarBorrador = (b: BorradorContrato) => {
    ...
    if (b.propiedadId && propiedadSigueDisponible) {
      ...
      if (!(pasoRestaurado >= 1 && pasoRestaurado <= 8)) pasoRestaurado = 1;
  ...
  const pasoDineroValido =
    ...
    // Vive en ESTE paso (el historial es plata, no plazo), así que gatea acá.
    !errorHistorialCanon;
  ...
  // ser otro campo (expensas, mora, cuenta o historial) el que falta, así que no
  // se puede asumir que si el paso está inválido siempre es culpa del alquiler.
  const mensajeDinero =
    requiereAlquiler && !(Number(monto) > 0)
      ...
      : incluyeExpensas && !(Number(montoExpensas) > 0)
        ...
        : !(moraSel === 'HEREDAR' || moraSel === 'SIN_MORA' || Number(moraValor) > 0)
          ...
          : faltaCuentaCobranzaDirecta
            ...
            : (errorHistorialCanon ?? 'Revisá los datos de este paso.');
  ...
  const avanzar = () =>
    setPaso((p) => {
      const sig = Math.min(8, p + 1) as PasoApi;
  ...
  // Resumen de deuda inicial (footer del paso 5 + resumen del confirmar). Cada
  ...
  // Si el usuario vuelve al paso 4 (Dinero) y cambia el esquema de mora, el monto
  // o el historial de canon, refrescamos las moras SUGERIDAS de los períodos que
  // no editó a mano.
  useEffect(() => {
    setPeriodosForm((forms) => {
      let cambio = false;
  ...
  return (
    <>
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 border-t pt-4">
                ...
                <div className="flex flex-wrap items-center gap-3">
                  {/* El motivo se lee. Un botón gris sin explicación deja al
                      operador buscando cuál de las 9 tarjetas le falta. Por eso
                      el botón NO lleva `disabled`: es clickeable y lo que hace es
                      marcar en rojo lo que falta (`faltantesMarcados`). */}
          ...
          <Card>
            ...
            <CardContent className="space-y-5">
              ...
              {/* Última compuerta: a este paso se puede llegar por un borrador
                  restaurado, que puede tener guardado `paso: 8` con meses todavía
      ...
      {/* Dos verdades, las dos ciertas, y ninguna se puede tirar:
          1) El texto viejo decía "vas a perder el progreso… no se puede deshacer",
             pero el onConfirm SÓLO navega: el borrador sigue guardado y al reabrir
             el alta vuelve todo. Mentía en la dirección más cara —asustaba con una
             pérdida que no ocurre— y escondía que el trabajo quedaba a salvo.
          2) Los servicios tampoco se pierden, pero por otro motivo: el paso 7
             escribe directo sobre la PROPIEDAD y ese PUT ya se ejecutó. */}
      <ConfirmDialog
        open={cancelarAbierto}
        onOpenChange={setCancelarAbierto}
        ...
        description={
          serviciosVisitados
            ? 'Lo cargado queda guardado como borrador: cuando vuelvas a entrar te lo ofrecemos para retomar. Los servicios que hayas guardado quedan en la propiedad: eso no se borra.'
            : 'Lo cargado queda guardado como borrador: cuando vuelvas a entrar te lo ofrecemos para retomar.'
        }
        ...
        <DialogContent variant="compact" className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              {/* Se muestra SIEMPRE, sin condicionar: los `File` nunca se
                  serializan a localStorage, así que el borrador ni siquiera sabe
                  si había archivos cargados — condicionarlo sería mentir.
                  Nombra al expediente entero (no sólo el DNI) porque el paso
                  Documentación adjunta también el contrato y los papeles de los
                  garantes, y todos se pierden igual. */}
              <span className="mt-2 block">
                Ojo: los archivos que hayas adjuntado (DNI, contrato, papeles de los garantes) no
                se guardan en el borrador, hay que volver a elegirlos.
              </span>
            </DialogDescription>
          </DialogHeader>
          {cargando && (
