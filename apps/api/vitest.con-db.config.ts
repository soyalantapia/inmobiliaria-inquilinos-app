...
// El criterio de partición vive en UN solo lugar (`test-particion.ts`) y lo importan las dos
// configs. Estaba duplicado y las copias divergieron: ésta buscaba `'seedBase'` pelado y la
// otra `'seedBase('` sobre el código sin comentarios, así que `guard-db.test.ts` corría en los
// dos jobs. `test/particion-de-suites.test.ts` verifica la propiedad — la prometía este
// comentario y ese test no existía.
